...
export function mountV2(opts: MountV2Options): V2Handles {
  ...
  const db = opts.db;
  // Wrap verifyToken so WS server gets null on bad tokens instead of a throw.
  const safeVerifyToken = (token: string): { userId: number } | null => {
    try { return verifyToken(token); } catch { return null; }
  };
  const wsServer = new GrvtWebSocketServer(opts.httpServer, {
    apiKey: opts.apiKey,
    verifyToken: safeVerifyToken,
  ...
  // Daily pruning of expired revoked_tokens rows. Rows past their
  // original JWT exp can never match a valid token — safe to delete.
  const PRUNE_INTERVAL_MS = 24 * 60 * 60 * 1000;
  const pruneTimer = setInterval(async () => {
    try {
      const pruned = await opts.gridBotDb.pruneExpiredRevokedTokens();
      if (pruned > 0) log.info({ pruned }, 'pruned expired revoked tokens');
    } catch (err) {
      log.warn({ err }, 'failed to prune revoked tokens');
    }
  }, PRUNE_INTERVAL_MS);
  pruneTimer.unref?.();
