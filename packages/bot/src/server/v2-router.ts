...
// set by the JWT middleware. Every protected handler reads req.userId.
...
function makeAuthMiddleware(apiKey: string, gBotDb: GridBotDB) {
  return (req: Request, res: Response, next: NextFunction) => {
    ...
    if (m) {
      ...
      if (payload) {
        // Check revocation asynchronously before granting access.
        gBotDb.isTokenRevoked(payload.jti).then((revoked) => {
          if (revoked) {
            log.warn({ ip: req.ip, jti: payload!.jti }, 'rejected revoked JWT');
          ...
          // DB error — fail open so a DB blip doesn't lock out everyone.
          req.userId = payload!.userId;
          req.jwtJti = payload!.jti;
          return next();
        });
...
// Trading mutations: 30 ops/min. Prevents scripts from hammering
// start/pause/close/create in a tight loop — even a legitimate user
// would never hit 30 creates/pauses in 60s manually.
const MUTATION_LIMITER = makeAuthLimiter(30, 60 * 1000);

// Minimum password requirements: at least 8 chars AND at least one digit
// or one special character. Returns an error string or null when valid.
...
Al hacer click en "Leí y acepto los términos de arriba" y crear una cuenta, confirmás que leíste, comprendiste y aceptás estar obligado por cada cláusula de arriba, que tenés al menos 18 años, que estás usando tus propios fondos y que aceptás todo el riesgo de pérdida.`;
  ...
  router.get('/metrics', (req: Request, res: Response, next: NextFunction) => {
    ...
    // When the caller passes a valid JWT, scope metrics to their own bots.
    // METRICS_TOKEN callers (Prometheus scrapers) see all bots.
    let scopeUserId: number | null = null;
    const metricsTokenPresent = (() => {
      const required = process.env.METRICS_TOKEN?.trim();
    ...
    if (!metricsTokenPresent) {
      const authHeader = req.header('authorization') || '';
      const jwtBearer = /^Bearer\s+(.+)$/i.exec(authHeader)?.[1];
      if (jwtBearer) {
        try {
          const payload = verifyToken(jwtBearer);
          scopeUserId = payload.userId;
        } catch {
          // Invalid JWT — scope stays null (localhost-allowed requests proceed unscoped)
        }
      }
    }

    const bots = await (scopeUserId != null
      ? dbAll<{
          ...
          position_size: number;
                       ...
                       grid_profit_usdt, trend_pnl_usdt, position_size
                FROM grid_bots WHERE COALESCE(user_id, 1) = ?`, [scopeUserId])
      : dbAll<{
          id: number; status: string; pair: string;
          investment_usdt: number; total_pnl_usdt: number;
          grid_profit_usdt: number; trend_pnl_usdt: number;
  ...
  // Revoke the calling user's JWT so it becomes invalid immediately.
  // Fire-and-forget from the frontend — a 200 means server-side
  // invalidated; the client must also clear its own localStorage token.
  router.post('/auth/logout', asyncHandler(async (req, res) => {
    const jti = req.jwtJti;
    const userId = req.userId!;
    if (jti) {
      // Decode expiry from the bearer token to set expires_at on the revoked row.
      const authHeader = req.header('authorization') || '';
      const bearer = /^Bearer\s+(.+)$/i.exec(authHeader)?.[1];
      let expiresAt = Date.now() + 24 * 60 * 60 * 1000; // fallback: now + 24h
      if (bearer) {
        try {
          const p = verifyToken(bearer);
          if (p.exp) expiresAt = p.exp * 1000;
        } catch { /* expired token — use fallback */ }
      }
      await gridBotDb.revokeToken(jti, userId, expiresAt);
    }
    res.json({ ok: true });
    return;
  }));
  ...
  // ── GET /api/v2/bots/:id/market-analysis ──────────────────────────
  // Returns ATR(14), market regime, and suggested grid range for the
  // bot's pair. Computed from last 100 hourly candles, cached 5 min.
  router.get('/bots/:id/market-analysis', asyncHandler(async (req, res) => {
    const id = parseInt(String(req.params.id ?? ''), 10);
    if (!Number.isFinite(id)) return res.status(400).json({ error: 'invalid bot id' });
    await requireBotOwnership(db, id, req.userId!);
    const bot = await dbGet<{ pair: string }>(db, `SELECT pair FROM grid_bots WHERE id = ?`, [id]);
    if (!bot) return res.status(404).json({ error: 'bot not found' });

    const { pair } = bot;
    const result = await cache.getOrFetch(`market-analysis:${pair}`, 5 * 60_000, async () => {
      const klines = (await grvtClient.getKlines(pair, 'CI_1_H', 100)) as Array<{
      ...
      const { lowerPrice, upperPrice } = suggestGridSpacing(atr, 10, currentPrice);
      ...
      return {
        ...
        suggestedSpacing: currentPrice > 0 ? (atr * 1.5) / 10 : 0,
  ...
  router.post('/admin/manual-trade', MUTATION_LIMITER, asyncHandler(async (req, res) => {
    // Admin only.
  ...
  router.post('/bots/validate', asyncHandler(async (req, res) => {
    ...
    if (body.run_backtest === true) {
      try {
        const klines = (await grvtClient.getKlines(pair, 'CI_1_H', 500)) as Array<{
          openTime: number; open: number; high: number; low: number; close: number;
        }>;
        const candles = klines.map((k) => ({
          time: k.openTime / 1000,
        ...
        const bt = runBacktest(
          { pair, direction, leverage, lowerPrice: lower, upperPrice: upper, numGrids: grids, investmentUSDT: investment },
          candles
        );
  ...
  router.post('/bots', MUTATION_LIMITER, asyncHandler(async (req, res) => {
    const body = (req.body ?? {}) as Partial<{
      pair: string;
      direction: 'long' | 'short';
      lower_price: number;
  ...
  router.post('/bots/:id/start', MUTATION_LIMITER, asyncHandler(async (req, res) => {
    const id = parseInt(String(req.params.id ?? ''), 10);
    if (!Number.isFinite(id)) return res.status(400).json({ error: 'invalid bot id' });
    await requireBotOwnership(db, id, req.userId!);
    try {
  ...
  // change.
  router.post('/bots/:id/pause', MUTATION_LIMITER, asyncHandler(async (req, res) => {
    const id = parseInt(String(req.params.id ?? ''), 10);
    if (!Number.isFinite(id)) return res.status(400).json({ error: 'invalid bot id' });
    await requireBotOwnership(db, id, req.userId!);
    try {
  ...
  router.post('/bots/:id/close', MUTATION_LIMITER, asyncHandler(async (req, res) => {
    const id = parseInt(String(req.params.id ?? ''), 10);
    if (!Number.isFinite(id)) return res.status(400).json({ error: 'invalid bot id' });
    await requireBotOwnership(db, id, req.userId!);
    try {
  ...
  // ── PATCH /api/v2/bots/:id/funding-alert ─────────────────────────
  // Set (or clear) the per-bot high-funding-rate alert threshold.
  // threshold_pct: percentage threshold (e.g. 0.1 means 0.1%/8h).
  // Send null or 0 to disable.
  router.patch('/bots/:id/funding-alert', asyncHandler(async (req, res) => {
    const id = parseInt(String(req.params.id ?? ''), 10);
    if (!Number.isFinite(id)) return res.status(400).json({ error: 'invalid bot id' });
    await requireBotOwnership(db, id, req.userId!);
    const { threshold_pct } = (req.body ?? {}) as { threshold_pct?: unknown };
    let value: number | null;
    if (threshold_pct == null || threshold_pct === 0) {
      value = null;
    } else if (typeof threshold_pct !== 'number' || threshold_pct <= 0 || threshold_pct > 100) {
      return res.status(400).json({ error: 'threshold_pct must be between 0 and 100 (or null to disable)' });
    } else {
      value = threshold_pct;
    }
    await gridBotDb.updateBot(id, { alert_funding_rate_pct: value });
    cache.invalidatePrefix('bots');
    res.json({ id, alert_funding_rate_pct: value });
    return;
  }));
  ...
  router.post('/bots/:id/range', MUTATION_LIMITER, asyncHandler(async (req, res) => {
    const id = parseInt(String(req.params.id ?? ''), 10);
    if (!Number.isFinite(id)) return res.status(400).json({ error: 'invalid bot id' });
    await requireBotOwnership(db, id, req.userId!);
