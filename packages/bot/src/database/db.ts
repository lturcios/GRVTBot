...
export class GridBotDB {
  ...
  private async runMultitenantMigration(): Promise<void> {
    ...
    // JWT revocation — rows here make a token invalid before it expires.
    // expires_at is stored so we can prune rows that are past their
    // original JWT lifetime (they can never match a valid token anyway).
    await this.dbRun(`
    ...
    `);
    await this.dbRun(`CREATE INDEX IF NOT EXISTS idx_revoked_expires ON revoked_tokens(expires_at)`);

    // alert_funding_rate_pct — per-bot high-funding alert threshold (Phase 3.4).
    try {
  ...
  async updateBot(botId: number, updates: Partial<GridBot>): Promise<void> {
    const ALLOWED_BOT_UPDATE_FIELDS: ReadonlySet<string> = new Set([
      'user_id', 'pair', 'direction', 'leverage', 'lower_price', 'upper_price',
      'num_grids', 'investment_usdt', 'grid_profit_usdt', 'trend_pnl_usdt',
      'total_pnl_usdt', 'status', 'position_size', 'avg_entry_price',
      'liquidation_price', 'params_json', 'grid_profit_seed',
      'grid_profit_seed_timestamp', 'total_reinvested', 'original_investment_usdt',
      'quantity_per_level', 'compound_pct', 'compound_threshold_usdt',
      'compound_interval_hours', 'last_compound_at', 'safeguard_enabled',
      'safeguard_threshold_pct', 'safeguard_action', 'alert_drawdown_pct',
      'alert_fill_batch', 'alert_liq_proximity_pct', 'sl_pct', 'tp_pct',
    ...
    for (const [key] of entries) {
      if (!ALLOWED_BOT_UPDATE_FIELDS.has(key)) {
        throw new Error(`updateBot: unknown field "${key}" — possible injection attempt`);
      }
    }
  ...
  async createGrvtSubAccount(params: {
    ...
    return new Promise((resolve, reject) => {
      db.serialize(() => {
        ...
        // Demote any existing default for this user atomically.
  ...
  async updateGrvtSubAccountMeta(
    ...
    return new Promise((resolve, reject) => {
      db.serialize(() => {
        db.run('BEGIN TRANSACTION');
        // Atomically demote all other defaults before promoting this row.
        if (patch.is_default === true) {
          db.run(
            `UPDATE grvt_sub_accounts SET is_default = 0 WHERE user_id = ? AND id != ?`,
            [userId, id],
            (err) => { if (err) { db.run('ROLLBACK'); return reject(err); } }
          );
        }
  ...
  async revokeToken(jti: string, userId: number, expiresAt: number): Promise<void> {
    await this.dbRun(
  ...
  async isTokenRevoked(jti: string): Promise<boolean> {
    const row = await this.dbGet(
      `SELECT 1 as found FROM revoked_tokens WHERE jti = ? AND expires_at > ?`,
      [jti, Date.now()]
    ) as { found: number } | undefined;
    return !!row?.found;
  }

  async pruneExpiredRevokedTokens(): Promise<number> {
    const result = await this.dbRun(
